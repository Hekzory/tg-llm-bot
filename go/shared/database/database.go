...
func (db *DB) AddUser(user *models.User) error {
	query := `INSERT INTO users (tg_id) VALUES ($1) RETURNING id, created_at`
	return db.QueryRowx(query, user.TelegramID).Scan(&user.ID, &user.CreatedAt)
}
...
func (db *DB) IsUserExist(user  *models.User) (bool, error) {
	var exist bool
